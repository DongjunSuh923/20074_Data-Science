...
def build_feature_table(feature_conn: sqlite3.Connection) -> None:
...
def summarize_table(conn: sqlite3.Connection, table: str) -> dict:
    return {
        ...
        "year_range": conn.execute(f"SELECT MIN(year), MAX(year) FROM {table}").fetchone(),
    }
...
def make_validation_artifacts(conn: sqlite3.Connection) -> dict:
    ...
    nan_query = """
    SELECT
        SUM(CASE WHEN tons_lag1 IS NULL THEN 1 ELSE 0 END),
        SUM(CASE WHEN tons_lag2 IS NULL THEN 1 ELSE 0 END),
        SUM(CASE WHEN tons_growth_rate IS NULL THEN 1 ELSE 0 END),
        SUM(CASE WHEN value_per_ton_lag1 IS NULL THEN 1 ELSE 0 END),
        SUM(CASE WHEN tons_per_tmile_lag1 IS NULL THEN 1 ELSE 0 END)
    FROM feature_all_rows;
    """
    ...
    extreme_query = """
    SELECT
        SUM(CASE WHEN ABS(COALESCE(tons_growth_rate, 0)) > 10 THEN 1 ELSE 0 END),
        SUM(CASE WHEN value_per_ton_lag1 < 0 THEN 1 ELSE 0 END),
        SUM(CASE WHEN tons_per_tmile_lag1 < 0 THEN 1 ELSE 0 END)
    ...
    summary["distribution_stats"] = {metric: stats(sample[metric]) for metric in metrics}

    plt.style.use("ggplot")
    for col, title, log_x in [
        ...
        plt.hist(clean.clip(lower=1e-9) if log_x else clean, bins=60, color="#33658a", alpha=0.9)
        if log_x:
            plt.xscale("log")
        plt.title(title)
        plt.xlabel(col)
    ...
    FEATURE_DOC_MD.write_text(
        "\n".join(
            [
                ...
                "- `target_log_tons`: `ln(1 + target_tons)` for skew-robust modeling.",
                "",
                "## Feature Groups",
                ...
                "- `orig_unique_dest_count_lag1` and `dest_unique_orig_count_lag1` proxy connectivity.",
                "- `route_share_of_origin_lag1` and `route_share_of_dest_lag1` proxy corridor dependence.",
                "- `corridor_trade_type_count_lag1` and `corridor_sctg2_count_lag1` describe route diversity.",
            ]
        ),
        encoding="utf-8",
    )
    ...
    SUMMARY_MD.write_text(
        "\n".join(
            [
                ...
                f"- Forecast 2025 CSV: `{FEATURE_2025_CSV}`",
...
def main() -> None:
    ...
    build_feature_table(feature_conn)
