...
NEXT_MUST_PATH = PROJECT_ROOT / "outputs" / "scenario_model" / "scenario_next15_hubs" / "scenario_next15_excluding_must_have.csv"
NEXT_BACKBONE_PATH = PROJECT_ROOT / "outputs" / "scenario_model" / "scenario_next15_backbone_only" / "scenario_next15_excluding_backbone_only.csv"
OUT_DIR = PROJECT_ROOT / "outputs" / "scenario_model" / "tier_a_hub_candidates_top15"


def main() -> None:
    OUT_DIR.mkdir(parents=True, exist_ok=True)
    must_have = set(pd.read_csv(MUST_HAVE_PATH)["state"].astype(str))
    a = pd.read_csv(NEXT_MUST_PATH)
    b = pd.read_csv(NEXT_BACKBONE_PATH)
    ...
    a_counts = a.groupby("state_abbr", as_index=False).agg(
        ...
        must_have_best_rank=("next_rank", "min"),
        must_have_avg_rank=("next_rank", "mean"),
    )
    b_counts = b_non_must.groupby("state_abbr", as_index=False).agg(
        backbone_only_scenario_count=("scenario_name", "nunique"),
        backbone_only_best_rank=("next_rank", "min"),
        backbone_only_avg_rank=("next_rank", "mean"),
    )

    overlap = a[["scenario_name", "state_abbr", "next_rank"]].merge(
        b_non_must[["scenario_name", "state_abbr", "next_rank"]],
    ...
    overlap_counts = overlap.groupby("state_abbr", as_index=False).agg(
        ...
        overlap_best_combined_rank=("next_rank_must", "min"),
    )
    ...
    lines = [
        "# Tier A Hub Candidate Rule (Top 15 Pool)",
        "",
        "Tier A is restricted to non-must-have states that survive both lenses:",
        "- must-have exclusion lens: still ranks inside next15 when the current core hubs are removed",
        "- backbone-only exclusion lens: still ranks inside next15 when only backbone hubs are removed",
        "",
