...
OUTPUT_DIR = PROJECT_ROOT / "outputs" / "models" / "macro_feature_impact"
METRICS_CSV = OUTPUT_DIR / "macro_model_metrics.csv"
COMPARISON_CSV = OUTPUT_DIR / "macro_vs_original_comparison.csv"
SUMMARY_JSON = OUTPUT_DIR / "macro_feature_impact_summary.json"
SUMMARY_MD = OUTPUT_DIR / "macro_feature_impact_summary.md"
...
def evaluate_persistence(df: pd.DataFrame) -> list[dict]:
    ...
    for split_name in SPLITS:
        ...
        rows.append(metric_dict("persistence_with_macro_dataset", split_name, split_df, pred))
    return rows
...
def evaluate_linear(df: pd.DataFrame) -> list[dict]:
    ...
    joblib.dump(pipeline, OUTPUT_DIR / "linear_with_macro.joblib")

    rows = []
    for split_name in SPLITS:
        split_df = df.loc[df["split"] == split_name].copy()
        pred = np.clip(pipeline.predict(split_df[CATEGORICAL_FEATURES + NUMERIC_FEATURES]), a_min=0, a_max=None)
        rows.append(metric_dict("linear_with_macro", split_name, split_df, pred))
    return rows


def tree_models() -> dict[str, object]:
    return {
        "random_forest_with_macro": RandomForestRegressor(
            n_estimators=250,
            max_depth=18,
            min_samples_leaf=2,
            n_jobs=-1,
        ...
        "xgboost_with_macro": XGBRegressor(
            n_estimators=400,
            max_depth=8,
            learning_rate=0.05,
            subsample=0.8,
        ...
        "lightgbm_with_macro": LGBMRegressor(
            n_estimators=400,
            learning_rate=0.05,
            num_leaves=63,
            subsample=0.8,
...
def load_original_metrics() -> pd.DataFrame:
    ...
    mapping = {
        "persistence_tons_lag1": "persistence_with_macro_dataset",
        "linear_full": "linear_with_macro",
        "random_forest": "random_forest_with_macro",
        "xgboost": "xgboost_with_macro",
        "lightgbm": "lightgbm_with_macro",
    }
    original["macro_model_name"] = original["model_name"].map(mapping)
    return original


def save_delta_plot(comparison_df: pd.DataFrame) -> None:
    plot_df = comparison_df.copy()
    plot_df["split_model"] = plot_df["macro_model_name"] + "\n" + plot_df["split"]
    plot_df = plot_df.sort_values(["macro_model_name", "split"])
    ...
    plt.ylabel("RMSE Delta (Macro - Original)")
    plt.title("Macro External Feature Impact on RMSE")
...
def write_summary(metrics_df: pd.DataFrame, comparison_df: pd.DataFrame, elapsed_sec: float) -> None:
    lines = [
        "# Macro Feature Impact",
        "",
        "## Setup",
        "- Input dataset: `feature_dataset_model_ready_with_macro.csv`",
        "- Added macro features: freight index, trucking price index, inventories, retail sales (annual mean/std/yoy)",
        "- Split policy: train=2020-2021, validation=2022, test_2023=2023, test_2024=2024",
        f"- Tree-model train sample cap: {TRAIN_SAMPLE_CAP:,}",
        "",
        "## Macro-Enhanced Metrics",
    ]
    ...
    for _, row in comparison_df.iterrows():
        ...
        lines.append(
            f"- {row['macro_model_name']} / {row['split']}: RMSE delta={row['rmse_delta']:.4f} ({direction}), MAE delta={row['mae_delta']:.4f}, R2 delta={row['r2_delta']:.4f}"
        )
...
def main() -> None:
    ...
    comparison_df = metrics_df.merge(
        ...
        right_on=["macro_model_name", "split"],
        how="left",
        suffixes=("_macro", "_original"),
    )
    comparison_df["rmse_delta"] = comparison_df["rmse_macro"] - comparison_df["rmse_original"]
    comparison_df["mae_delta"] = comparison_df["mae_macro"] - comparison_df["mae_original"]
    comparison_df["r2_delta"] = comparison_df["r2_macro"] - comparison_df["r2_original"]
    ...
    summary = {
        ...
        "best_validation_rmse_with_macro": metrics_df.loc[metrics_df["split"] == "validation"].sort_values("rmse").iloc[0].to_dict(),
    }
