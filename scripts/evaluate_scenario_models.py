...
RESULTS_PATH = OUTPUT_DIR / "scenario_model_results.csv"
SUMMARY_PATH = OUTPUT_DIR / "scenario_model_summary.md"
SUMMARY_JSON = OUTPUT_DIR / "scenario_model_summary.json"
...
def main() -> None:
    ...
    lines = [
        ...
        "",
        "- Scenario dataset uses state-to-state + commodity + distance + trade_type + year aggregation.",
        "- Same-route lag features were removed.",
        "- Target remains tons (and log1p(tons) for comparison).",
        "",
        "## Best by split",
    ]
    for _, row in best.iterrows():
        lines.append(
            f"- {row['split']}: {row['model_name']} ({row['target_name']}) RMSE={row['rmse']:.4f}, MAE={row['mae']:.4f}, R2={row['r2']:.4f}"
        )
