...
OUTPUT_DIR = PROJECT_ROOT / "outputs" / "models" / "state_external_impact"
METRICS_CSV = OUTPUT_DIR / "state_external_model_metrics.csv"
COMPARISON_CSV = OUTPUT_DIR / "state_external_vs_original_comparison.csv"
SUMMARY_JSON = OUTPUT_DIR / "state_external_impact_summary.json"
SUMMARY_MD = OUTPUT_DIR / "state_external_impact_summary.md"
...
def evaluate_persistence(df: pd.DataFrame) -> list[dict]:
    ...
    for split_name in SPLITS:
        ...
        rows.append(metric_dict("persistence_with_state_external", split_name, split_df, pred))
    return rows
...
def evaluate_linear(df: pd.DataFrame) -> list[dict]:
    ...
    pipeline.fit(train_df[CATEGORICAL_FEATURES + NUMERIC_FEATURES], train_df["target_tons"].to_numpy())
    joblib.dump(pipeline, OUTPUT_DIR / "linear_with_state_external.joblib")

    rows = []
    for split_name in SPLITS:
        split_df = df.loc[df["split"] == split_name].copy()
        pred = np.clip(pipeline.predict(split_df[CATEGORICAL_FEATURES + NUMERIC_FEATURES]), a_min=0, a_max=None)
        rows.append(metric_dict("linear_with_state_external", split_name, split_df, pred))
    return rows


def tree_models() -> dict[str, object]:
    return {
        "random_forest_with_state_external": RandomForestRegressor(
            n_estimators=250,
            max_depth=18,
            min_samples_leaf=2,
            n_jobs=-1,
        ...
        "xgboost_with_state_external": XGBRegressor(
            n_estimators=400,
            max_depth=8,
            learning_rate=0.05,
            subsample=0.8,
        ...
        "lightgbm_with_state_external": LGBMRegressor(
            n_estimators=400,
            learning_rate=0.05,
            num_leaves=63,
            subsample=0.8,
...
def load_original_metrics() -> pd.DataFrame:
    ...
    mapping = {
        "persistence_tons_lag1": "persistence_with_state_external",
        "linear_full": "linear_with_state_external",
        "random_forest": "random_forest_with_state_external",
        "xgboost": "xgboost_with_state_external",
        "lightgbm": "lightgbm_with_state_external",
    }
    original["state_external_model_name"] = original["model_name"].map(mapping)
    return original


def save_delta_plot(comparison_df: pd.DataFrame) -> None:
    plot_df = comparison_df.copy()
    plot_df["split_model"] = plot_df["state_external_model_name"] + "\n" + plot_df["split"]
    plot_df = plot_df.sort_values(["state_external_model_name", "split"])
    ...
    plt.ylabel("RMSE Delta (State External - Original)")
    plt.title("State-Level External Feature Impact on RMSE")
...
def write_summary(metrics_df: pd.DataFrame, comparison_df: pd.DataFrame, elapsed_sec: float) -> None:
    lines = [
        "# State External Feature Impact",
        "",
        "## Setup",
        "- Input dataset: `feature_dataset_model_ready_with_state_external.csv`",
        "- Added state-level external features: GDP + CBP employment/establishment features",
        "- Split policy: train=2020-2021, validation=2022, test_2023=2023, test_2024=2024",
        f"- Tree-model train sample cap: {TRAIN_SAMPLE_CAP:,}",
        "",
        "## State-Enhanced Metrics",
    ]
    for _, row in metrics_df.iterrows():
        lines.append(
            f"- {row['model_name']} / {row['split']}: RMSE={row['rmse']:.4f}, MAE={row['mae']:.4f}, R2={row['r2']:.4f}"
        )
    lines += ["", "## Comparison With Original Models"]
    for _, row in comparison_df.iterrows():
        direction = "improved" if row["rmse_delta"] < 0 else "worsened"
        lines.append(
            f"- {row['state_external_model_name']} / {row['split']}: RMSE delta={row['rmse_delta']:.4f} ({direction}), MAE delta={row['mae_delta']:.4f}, R2 delta={row['r2_delta']:.4f}"
        )
...
def main() -> None:
    ...
    comparison_df = metrics_df.merge(
        ...
        right_on=["state_external_model_name", "split"],
        how="left",
        suffixes=("_state", "_original"),
    )
    comparison_df["rmse_delta"] = comparison_df["rmse_state"] - comparison_df["rmse_original"]
    comparison_df["mae_delta"] = comparison_df["mae_state"] - comparison_df["mae_original"]
    comparison_df["r2_delta"] = comparison_df["r2_state"] - comparison_df["r2_original"]
    ...
    summary = {
        ...
        "best_validation_rmse_with_state_external": metrics_df.loc[metrics_df["split"] == "validation"].sort_values("rmse").iloc[0].to_dict(),
    }
