...
CONSTRUCTION_NEXT15 = PROJECT_ROOT / "outputs" / "scenario_model" / "domestic_issue_hybrid" / "construction_infra_hybrid_next15_excluding_must_have.csv"
...
COMMON_OVERLAP_PATH = WORK_ROOT / "outputs" / "scenario_model" / "domestic_issue_visuals" / "domestic_issue_candidate_overlap.csv"
OUT_DIR = WORK_ROOT / "outputs" / "scenario_model" / "domestic_issue_overlap_maps"
...
def build_sets() -> dict[str, set[str]]:
    ...
    construction = must_have | set(pd.read_csv(CONSTRUCTION_NEXT15)["state_abbr"].astype(str))
    bottleneck = must_have | set(pd.read_csv(BOTTLENECK_NEXT15)["state_abbr"].astype(str))
    overlap_df = pd.read_csv(COMMON_OVERLAP_PATH)
    common_states = must_have | set(overlap_df.loc[overlap_df["overlap_count"] == 2, "state_abbr"].astype(str))
    return {
...
def make_static_panel(sets: dict[str, set[str]]) -> None:
    color_map = {
        "Overlap": "#2563EB",
        "Scenario-only": "#16A34A",
        "NGL-only": "#F59E0B",
    ...
    titles = [
        ("01_construction_ngl_overlap", "Construction / Infrastructure vs NGL", sets["construction"]),
        ("02_bottleneck_ngl_overlap", "Domestic Bottleneck vs NGL", sets["bottleneck"]),
        ("03_common_domestic_ngl_overlap", "Common Domestic Candidates vs NGL", sets["common"]),
    ]

    fig, axes = plt.subplots(1, 3, figsize=(18, 7))
    for ax, (_, title, scenario_states) in zip(axes, titles):
    ...
    fig.suptitle("NGL Portfolio vs Domestic Issue Simulation Results", fontsize=16, weight="bold", y=0.97)
    fig.tight_layout(rect=[0, 0.06, 1, 0.94])
    fig.savefig(OUT_DIR / "04_domestic_issue_ngl_static_comparison.png", dpi=240, bbox_inches="tight")
    plt.close(fig)

    for name, title, scenario_states in titles:
...
def write_summary(sets: dict[str, set[str]]) -> None:
    lines = ["# Domestic Issue vs NGL Overlap Summary", ""]
    for key, title in [
        ("construction", "Construction / Infrastructure"),
        ("bottleneck", "Domestic Bottleneck"),
        ("common", "Common Domestic Candidates"),
    ]:
    ...
    (OUT_DIR / "domestic_issue_ngl_overlap_summary.md").write_text("\n".join(lines), encoding="utf-8")


def main() -> None:
    OUT_DIR.mkdir(parents=True, exist_ok=True)
    sets = build_sets()
    make_html_map("01_construction_ngl_overlap", "NGL vs Construction/Infrastructure Domestic Shock", sets["construction"])
    make_html_map("02_bottleneck_ngl_overlap", "NGL vs Domestic Bottleneck Shock", sets["bottleneck"])
    make_html_map("03_common_domestic_ngl_overlap", "NGL vs Common Domestic-Issue Candidates", sets["common"])
    make_static_panel(sets)
    write_summary(sets)
    metadata = {
        "construction_html": str(OUT_DIR / "01_construction_ngl_overlap.html"),
        "bottleneck_html": str(OUT_DIR / "02_bottleneck_ngl_overlap.html"),
        "common_html": str(OUT_DIR / "03_common_domestic_ngl_overlap.html"),
        "static_panel_png": str(OUT_DIR / "04_domestic_issue_ngl_static_comparison.png"),
        "summary_md": str(OUT_DIR / "domestic_issue_ngl_overlap_summary.md"),
    }
    (OUT_DIR / "domestic_issue_ngl_overlap_metadata.json").write_text(json.dumps(metadata, ensure_ascii=False, indent=2), encoding="utf-8")
