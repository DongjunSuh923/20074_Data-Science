...
SCENARIO_FILES = {
    "Construction / Infrastructure": IDEA_ROOT / "outputs" / "scenario_model" / "domestic_issue_hybrid" / "construction_infra_hybrid_next15_excluding_must_have.csv",
    "Bottleneck / Congestion": DOWNLOAD_ROOT / "outputs" / "scenario_model" / "domestic_bottleneck_shock" / "domestic_bottleneck_next15_excluding_must_have.csv",
    "Broad Domestic Demand": DOWNLOAD_ROOT / "outputs" / "scenario_model" / "domestic_broad_demand_shock" / "simulation_results" / "domestic_broad_demand_next15_excluding_must_have.csv",
}
...
def main() -> None:
    ...
    for state, color in [("NC", "#F28E2B"), ("VA", "#F28E2B")]:
        if state in STATE_CENTROIDS:
            fig.add_trace(
                go.Scattergeo(
                    lat=[STATE_CENTROIDS[state][0]],
                    lon=[STATE_CENTROIDS[state][1]],
                    mode="markers+text",
                    text=[f"{state} ({STATE_KR[state]})"],
                    ...
                    hovertemplate=f"{state}: repeated domestic reinforcement candidate<extra></extra>",
                    showlegend=False,
                )
            )

    fig.update_layout(
        title="Domestic-Issue Track: Repeated Reinforcement Candidates",
        ...
        annotations=[
            dict(
                ...
                xref="paper",
                yref="paper",
                text="NC and VA are highlighted because they survive across construction, bottleneck, and broad-demand shocks.",
                showarrow=False,
    ...
    html_path = OUT_DIR / "08_domestic_issue_reinforcement_map.html"
