...
OUT_DIR = PROJECT_ROOT / "outputs" / "scenario_model" / "ngl_vs_scenario_overlap_maps"
...
def html_map(scenario_final: set[str]) -> None:
    ...
    fig.update_layout(
        title="NGL Current vs Scenario Final Hub Placement (Overlap Map)",
    ...
    fig.write_html(str(OUT_DIR / "01_ngl_vs_scenario_overlap_map.html"), include_plotlyjs="cdn")
...
def static_map(scenario_final: set[str], must_have: set[str], tier_a: set[str]) -> None:
    ...
    ax.set_title("NGL Current vs Scenario Final Hub Placement", fontsize=15, weight="bold")
    ...
    fig.tight_layout()
    fig.savefig(OUT_DIR / "02_ngl_vs_scenario_overlap_map.png", dpi=240, bbox_inches="tight")
    plt.close(fig)
...
def summary_text(scenario_final: set[str], must_have: set[str], tier_a: set[str]) -> None:
    ...
    lines = [
        "# NGL vs Scenario Final Overlap Summary",
        "",
        ...
        "- tier A: " + ", ".join(sorted(tier_a)),
    ]
    (OUT_DIR / "ngl_vs_scenario_overlap_summary.md").write_text("\n".join(lines), encoding="utf-8")
...
def main() -> None:
    ...
    metadata = {
        "html": str(OUT_DIR / "01_ngl_vs_scenario_overlap_map.html"),
        "png": str(OUT_DIR / "02_ngl_vs_scenario_overlap_map.png"),
        "summary": str(OUT_DIR / "ngl_vs_scenario_overlap_summary.md"),
    }
    (OUT_DIR / "ngl_vs_scenario_overlap_metadata.json").write_text(json.dumps(metadata, ensure_ascii=False, indent=2), encoding="utf-8")
