...
OUT_DIR = WORK_ROOT / "outputs" / "scenario_model" / "trade_external_overlap_maps"
...
def build_set() -> tuple[set[str], set[str], set[str]]:
    ...
    next15 = set(next15.loc[next15["scenario_name"] == "trade_external_state_blocking", "state_abbr"].astype(str))
...
def make_html_map(scenario_states: set[str]) -> None:
    ...
    fig.update_layout(
        title="NGL Current vs Trade / External Shock Candidate Placement",
    ...
    fig.write_html(str(OUT_DIR / "01_trade_external_ngl_overlap.html"), include_plotlyjs="cdn")
...
def make_static_map(scenario_states: set[str], must_have: set[str], next15: set[str]) -> None:
    ...
    ax.set_title("NGL Current vs Trade / External Shock Candidates", fontsize=15, weight="bold")
    ax.legend(handles=handles, loc="lower left", frameon=False, fontsize=9)
    fig.tight_layout()
    fig.savefig(OUT_DIR / "02_trade_external_ngl_overlap.png", dpi=240, bbox_inches="tight")
    plt.close(fig)
...
def write_summary(must_have: set[str], next15: set[str], scenario_states: set[str]) -> None:
    ...
    lines = [
        "# Trade / External Shock vs NGL Overlap Summary",
        "",
        ...
        "- trade next15: " + ", ".join(sorted(next15)),
    ]
    (OUT_DIR / "trade_external_ngl_overlap_summary.md").write_text("\n".join(lines), encoding="utf-8")
...
def main() -> None:
    ...
    metadata = {
        "html": str(OUT_DIR / "01_trade_external_ngl_overlap.html"),
        "png": str(OUT_DIR / "02_trade_external_ngl_overlap.png"),
        "summary": str(OUT_DIR / "trade_external_ngl_overlap_summary.md"),
    }
    (OUT_DIR / "trade_external_ngl_overlap_metadata.json").write_text(json.dumps(metadata, ensure_ascii=False, indent=2), encoding="utf-8")
