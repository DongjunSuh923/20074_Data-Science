...
TRADE_STATE_SCORES = PROJECT_ROOT / "outputs" / "scenario_model" / "trade_external_shock" / "trade_external_state_scores.csv"
...
TRADE_NETWORK_TOTALS = PROJECT_ROOT / "outputs" / "scenario_model" / "trade_external_shock" / "simulation_results" / "trade_external_network_totals.csv"
TRADE_INTERSTATE_TOTALS = PROJECT_ROOT / "outputs" / "scenario_model" / "trade_external_shock" / "simulation_results" / "trade_external_interstate_totals.csv"
...
def plot_loss_comparison(network: pd.DataFrame, interstate: pd.DataFrame) -> None:
    fig, ax = plt.subplots(figsize=(10, 5.5))
    ax.plot(network["year"], network["loss_pct"] * 100, marker="o", linewidth=2.2, color="#2563EB", label="Total network loss")
    ax.plot(interstate["year"], interstate["loss_pct"] * 100, marker="s", linewidth=2.2, color="#DC2626", label="Interstate-only loss")
    ax.axhline(0, color="#9CA3AF", linewidth=1)
    ax.set_title("Trade / External Network Shock: Annual Loss Profile", fontsize=14, weight="bold")
    ax.set_xlabel("Year")
    ax.set_ylabel("Loss (%)")
    ax.legend(frameon=False)
    ax.grid(axis="y", alpha=0.25)
    fig.tight_layout()
    fig.savefig(OUT_DIR / "01_trade_external_loss_profile.png", dpi=240, bbox_inches="tight")
    plt.close(fig)
...
def plot_state_selection_map(cases: pd.DataFrame) -> None:
    ...
    ax.set_title("Trade / External Network Shock: Selected Blocking States", fontsize=14, weight="bold")
    handles = [
    ...
    fig.tight_layout()
    fig.savefig(OUT_DIR / "02_trade_external_state_selection_map.png", dpi=240, bbox_inches="tight")
    plt.close(fig)
...
def make_state_selection_html(cases: pd.DataFrame) -> None:
    ...
    fig.update_layout(
        title="Trade / External Network Shock: Selected Blocking States",
    ...
    fig.write_html(str(OUT_DIR / "02_trade_external_state_selection_map.html"), include_plotlyjs="cdn")


def plot_next15(next15: pd.DataFrame) -> None:
    fig, ax = plt.subplots(figsize=(11, 5.5))
    ax.bar(next15["state_abbr"], next15["touch_2031"], color="#0F766E")
    ax.set_title("Trade Shock: Next 15 Hubs Excluding Must-have (2031 touch)", fontsize=14, weight="bold")
    ax.set_xlabel("State")
    ax.set_ylabel("2031 touch")
    ax.grid(axis="y", alpha=0.25)
    fig.tight_layout()
    fig.savefig(OUT_DIR / "03_trade_external_next15_touch.png", dpi=240, bbox_inches="tight")
    plt.close(fig)
...
def write_summary(summary: pd.Series, cases: pd.DataFrame, scores: pd.DataFrame, next15: pd.DataFrame) -> None:
    ...
    top5 = scores.sort_values("trade_external_score", ascending=False).head(5)
    lines = [
        "# Trade / External Network Shock Progress Reference",
        "",
        "## Shock setup",
        "",
        ...
        "- state selection logic: official Census state export exposure + vessel/container dependence + China exposure + interstate touch dependency",
        "",
        "## Aggregate outcome",
        "",
    ...
    for row in top5.itertuples(index=False):
        lines.append(
            f"- `{row.state_abbr}` {row.severity}: exports `${row.total_export_value_usd:,.0f}`, "
            f"China share `{row.china_share:.1%}`, vessel share `{row.vessel_share:.1%}`, "
            f"container share `{row.container_share:.1%}`"
        )
    lines.extend(
        [
            "",
            ...
            "",
            "- This shock stresses interstate trade reliability more than broad statewide demand.",
            "- `WA`, `LA`, `NC`, `OR`, `OK`, `NE` repeat near the top, suggesting coastal trade support and inland rerouting value.",
            "- `NC` and `VA` remain visible because East Coast corridor access and domestic distribution capacity overlap with trade exposure.",
        ]
    )
    (OUT_DIR / "TRADE_EXTERNAL_PROGRESS_REFERENCE.md").write_text("\n".join(lines), encoding="utf-8")


def main() -> None:
    OUT_DIR.mkdir(parents=True, exist_ok=True)
    cases = pd.read_csv(TRADE_CASES)
    scores = pd.read_csv(TRADE_STATE_SCORES)
    summary = pd.read_csv(TRADE_SUMMARY).iloc[0]
    network = pd.read_csv(TRADE_NETWORK_TOTALS)
    interstate = pd.read_csv(TRADE_INTERSTATE_TOTALS)
    next15 = pd.read_csv(TRADE_NEXT15)
    next15 = next15.loc[next15["scenario_name"] == "trade_external_state_blocking"].copy()
    next15["touch_2031"] = pd.to_numeric(next15["touch_2031"], errors="coerce")

    plot_loss_comparison(network, interstate)
    ...
    metadata = {
        "loss_profile_png": str(OUT_DIR / "01_trade_external_loss_profile.png"),
        "selection_map_png": str(OUT_DIR / "02_trade_external_state_selection_map.png"),
        "selection_map_html": str(OUT_DIR / "02_trade_external_state_selection_map.html"),
        "next15_png": str(OUT_DIR / "03_trade_external_next15_touch.png"),
        "summary_md": str(OUT_DIR / "TRADE_EXTERNAL_PROGRESS_REFERENCE.md"),
    }
    (OUT_DIR / "trade_external_visuals_metadata.json").write_text(json.dumps(metadata, ensure_ascii=False, indent=2), encoding="utf-8")
