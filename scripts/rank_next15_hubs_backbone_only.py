...
OUT_DIR = PROJECT_ROOT / "outputs" / "scenario_model" / "scenario_next15_backbone_only"
TOP_N = 15
...
def main() -> None:
    ...
    baseline_next = base_rank.loc[~base_rank["state_abbr"].isin(set(backbone_only["state_abbr"]))].head(TOP_N).copy()
    baseline_next["scenario_name"] = "baseline_no_shock"
    baseline_next["next_rank"] = np.arange(1, len(baseline_next) + 1)
    all_next = [baseline_next]
    ...
    for spec in SCENARIOS:
        ...
        next_states = summary.loc[~summary["state_abbr"].isin(set(backbone_only["state_abbr"]))].head(TOP_N).copy()
        next_states["next_rank"] = np.arange(1, len(next_states) + 1)
        next_states.to_csv(OUT_DIR / f"{spec['scenario_name']}_next{TOP_N}_excluding_backbone_only.csv", index=False)
        all_next.append(next_states)

    combined = pd.concat(all_next, ignore_index=True, sort=False)
    combined.to_csv(OUT_DIR / f"scenario_next{TOP_N}_excluding_backbone_only.csv", index=False)

    lines = [
        f"# Backbone-only Exclusion Validation (Next {TOP_N})",
        "",
        ...
        "",
        f"Baseline next {TOP_N} after excluding backbone-only set:",
        ...
        json.dumps(
            {
                ...
                "scenario_next": str(OUT_DIR / f"scenario_next{TOP_N}_excluding_backbone_only.csv"),
