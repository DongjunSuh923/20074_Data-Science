...
OUT_DIR = PROJECT_ROOT / "outputs" / "scenario_model" / "scenario_next15_hubs"
TOP_N = 15
...
def main() -> None:
    ...
    baseline_next = baseline_rank.loc[~baseline_rank["state_abbr"].isin(must_have)].head(TOP_N).copy()
    baseline_next["scenario_name"] = "baseline_no_shock"

    all_next_rows = [baseline_next.assign(next_rank=np.arange(1, len(baseline_next) + 1))]
    all_rank_rows = []

    for spec in SCENARIOS:
        cases = load_cases(spec["case_path"], spec["scenario_name"])
        shocked = apply_state_blocking(routes, cases, spec["commodity_filter"])
        shocked.to_csv(OUT_DIR / f"{spec['scenario_name']}_shocked_routes_2025_2031.csv", index=False)
        ...
        next_states = summary.loc[~summary["state_abbr"].isin(must_have)].head(TOP_N).copy()
        next_states["next_rank"] = np.arange(1, len(next_states) + 1)
        next_states.to_csv(OUT_DIR / f"{spec['scenario_name']}_next{TOP_N}_excluding_must_have.csv", index=False)
        all_next_rows.append(next_states)

    combined_next = pd.concat(all_next_rows, ignore_index=True, sort=False)
    combined_next.to_csv(OUT_DIR / f"scenario_next{TOP_N}_excluding_must_have.csv", index=False)
    ...
    for scenario_name in combined_next["scenario_name"].dropna().unique():
        if scenario_name == "baseline_no_shock":
            continue
        sub = combined_next.loc[combined_next["scenario_name"] == scenario_name].copy()
    ...
    comparison.to_csv(OUT_DIR / f"scenario_next{TOP_N}_rank_shift_vs_baseline.csv", index=False)

    lines = [
        f"# Scenario Next {TOP_N} Hub Ranking (Must-Have Fixed)",
        "",
        "Must-have hubs fixed and excluded from the candidate ranking:",
        f"- {', '.join(sorted(must_have))}",
        "",
        f"Baseline next {TOP_N} after excluding must-have:",
        "- " + ", ".join(baseline_next["state_abbr"].tolist()),
        "",
    ]
    for scenario_name in [s["scenario_name"] for s in SCENARIOS]:
        sub = combined_next.loc[combined_next["scenario_name"] == scenario_name]
        lines.append(f"## {scenario_name}")
        lines.append("- " + ", ".join(sub["state_abbr"].tolist()))
        leaders = comparison.loc[comparison["scenario_name"] == scenario_name].sort_values(["rank_shift_vs_baseline", "next_rank"], ascending=[False, True]).head(5)
        if not leaders.empty:
            lines.append("Promoted vs baseline:")
            for row in leaders.itertuples(index=False):
                lines.append(
                    f"- {row.state_abbr}: baseline_rank={row.baseline_hub_rank_2031}, "
                    f"scenario_rank={row.scenario_hub_rank_2031}, shift={row.rank_shift_vs_baseline:+d}, type={row.hub_type}"
                )
        lines.append("")

    (OUT_DIR / f"scenario_next{TOP_N}_summary.md").write_text("\n".join(lines), encoding="utf-8")
    metadata = {
        "combined_next": str(OUT_DIR / f"scenario_next{TOP_N}_excluding_must_have.csv"),
        "all_rankings": str(OUT_DIR / "scenario_all_hub_rankings_2031.csv"),
        "rank_shift": str(OUT_DIR / f"scenario_next{TOP_N}_rank_shift_vs_baseline.csv"),
        "summary": str(OUT_DIR / f"scenario_next{TOP_N}_summary.md"),
    }
    (OUT_DIR / f"scenario_next{TOP_N}_metadata.json").write_text(json.dumps(metadata, ensure_ascii=False, indent=2), encoding="utf-8")
