...
OUT_DIR = PROJECT_ROOT / "outputs" / "scenario_model" / "construction_infra_shock" / "simulation_results"
...
def main() -> None:
    ...
    for row in cases.itertuples(index=False):
        ...
        manifest.append({"state_fips": row.state_fips, "severity": row.severity, "schedule": sc})
    ...
    yearly.to_csv(OUT_DIR / "construction_infra_network_totals.csv", index=False)

    subset_base = routes.loc[routes["sctg2"].isin(commodity_set)].groupby("year", as_index=False)["prediction"].sum().rename(columns={"prediction": "base_subset_total"})
    subset_shock = shocked.loc[shocked["sctg2"].isin(commodity_set)].groupby("year", as_index=False)["prediction"].sum().rename(columns={"prediction": "shock_subset_total"})
    subset = subset_base.merge(subset_shock, on="year", how="left")
    subset["loss_abs"] = subset["base_subset_total"] - subset["shock_subset_total"]
    subset["loss_pct"] = subset["loss_abs"] / subset["base_subset_total"]
    subset.to_csv(OUT_DIR / "construction_infra_subset_totals.csv", index=False)
    ...
    summary = {
        ...
        "subset_cumulative_loss_pct_of_annual_sum": float(subset["loss_abs"].sum() / subset["base_subset_total"].sum()),
        "subset_peak_year_loss_pct": float(subset["loss_pct"].max()),
    }
    pd.DataFrame([summary]).to_csv(OUT_DIR / "construction_infra_simulation_summary.csv", index=False)
    (OUT_DIR / "construction_infra_simulation_manifest.json").write_text(json.dumps({"states": manifest, "summary": summary}, ensure_ascii=False, indent=2), encoding="utf-8")
    lines = [
        "# Construction / Infrastructure State Blocking Simulation Summary",
        "",
        ...
        f"- subset cumulative loss: {summary['subset_cumulative_loss_pct_of_annual_sum']:.2%}",
        f"- subset peak loss: {summary['subset_peak_year_loss_pct']:.2%}",
    ]
    (OUT_DIR / "construction_infra_simulation_summary.md").write_text("\n".join(lines), encoding="utf-8")
