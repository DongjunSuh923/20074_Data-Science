...
CASE_PATH = WORK_ROOT / "outputs" / "scenario_model" / "domestic_broad_demand_shock" / "domestic_broad_demand_state_blocking_cases.csv"
MUST_HAVE_PATH = IDEA_ROOT / "outputs" / "scenario_model" / "state_blocking_resilience" / "provisional_must_have_hubs.csv"
OUT_DIR = WORK_ROOT / "outputs" / "scenario_model" / "domestic_broad_demand_shock" / "simulation_results"
START_YEAR = 2026
TOP_N = 15
...
def main() -> None:
    ...
    for row in cases.itertuples(index=False):
        dest_mask = interstate_mask & (shocked["dest_state_fips"] == row.state_fips)
        for year, loss in schedule(row.severity).items():
            mask = dest_mask & (shocked["year"] == year)
    ...
    summary = {
        "scenario_name": "domestic_broad_demand_state_blocking",
        "blocked_states": ",".join(cases["state_fips"].astype(str).tolist()),
    ...
    yearly.to_csv(OUT_DIR / "domestic_broad_demand_network_totals.csv", index=False)
    interstate.to_csv(OUT_DIR / "domestic_broad_demand_interstate_totals.csv", index=False)
    rankings.to_csv(OUT_DIR / "domestic_broad_demand_hub_rankings_2031.csv", index=False)
    next15.to_csv(OUT_DIR / "domestic_broad_demand_next15_excluding_must_have.csv", index=False)
    pd.DataFrame([summary]).to_csv(OUT_DIR / "domestic_broad_demand_summary.csv", index=False)
    (OUT_DIR / "domestic_broad_demand_summary.md").write_text(
        "\n".join(
            [
                "# Domestic Broad Demand Shock Summary",
                "",
    ...
    (OUT_DIR / "domestic_broad_demand_metadata.json").write_text(
        json.dumps(
            {
                "summary": summary,
                "next15_csv": str(OUT_DIR / "domestic_broad_demand_next15_excluding_must_have.csv"),
                "rankings_csv": str(OUT_DIR / "domestic_broad_demand_hub_rankings_2031.csv"),
            },
            ensure_ascii=False,
            indent=2,
        ),
